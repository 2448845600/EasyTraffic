...
class STID(nn.Module):
    ...
    def forward(self, input_data):
        ...
        hidden = self.ts_emb_layer(input_data[..., 0])
        ...
        if self.tod_emb_dim:
            tod_data = (input_data[:, -1, :, 1]).type(torch.LongTensor)
            tod_emb = self.tod_emb[tod_data].transpose(1, 2)  # (B, tod_emb_dim, N)
            hidden = torch.cat((hidden, tod_emb), dim=1)

        if self.dow_emb_dim:
            dow_data = (input_data[:, -1, :, 2]).type(torch.LongTensor)
            dow_emb = self.dow_emb[dow_data].transpose(1, 2)
